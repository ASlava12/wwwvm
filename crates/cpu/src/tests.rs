...
/// LGDT from CPL=3 raises #GP(0). Real silicon classifies the
/// descriptor-table-loading instructions (LGDT/LIDT/LLDT/LTR) as
/// supervisor-only — userspace LGDT must trap so a malicious
/// process can't relocate the kernel's GDT.
#[test]
fn lgdt_in_ring_3_raises_gp() {
    let mut mem = Memory::new(0x10_0000);
    let gdt: [u8; 48] = [
        0, 0, 0, 0, 0, 0, 0, 0, 0xFF, 0xFF, 0x00, 0x00, 0x00, 0x9A, 0xCF, 0x00, 0xFF, 0xFF, 0x00,
        0x00, 0x00, 0x92, 0xCF, 0x00, 0xFF, 0xFF, 0x00, 0x00, 0x00, 0xFA, 0xCF, 0x00, 0xFF, 0xFF,
    ...
    // Pseudo-descriptor at 0x8100 (limit=0, base=0) — won't actually
    // load because the #GP fires first.
    mem.write_slice(0x8100, &[0, 0, 0, 0, 0, 0]);
    // User-mode LGDT [0x8100]:
    //   0F 01 15 00 81 00 00  — opcode 0F 01, modrm=00 010 101 (sub=2, [disp32]), disp32=0x8100.
    mem.write_slice(0x8000, &[0x0F, 0x01, 0x15, 0x00, 0x81, 0x00, 0x00]);
    ...
    let mut io = IoBus::new();
    cpu.step(&mut mem, &mut io).expect("step LGDT");
    ...
    assert_eq!(cpu.ip, 0x9000);
    // GDTR untouched.
    assert_eq!(cpu.gdtr.base, 0x0500);
    assert_eq!(cpu.gdtr.limit, 0x2F);
    // Saved IP names the start of LGDT (fault, not trap).
    assert_eq!(mem.read_u32(0x3000 - 20), 0x8000);
}
...
/// state. Linux's `rdmsr_safe` already expects #GP for unknown
/// MSRs; the user-mode invocation falls into the same arm.
#[test]
fn rdmsr_in_ring_3_raises_gp() {
    let mut mem = Memory::new(0x10_0000);
    let gdt: [u8; 48] = [
        0, 0, 0, 0, 0, 0, 0, 0, 0xFF, 0xFF, 0x00, 0x00, 0x00, 0x9A, 0xCF, 0x00, 0xFF, 0xFF, 0x00,
        0x00, 0x00, 0x92, 0xCF, 0x00, 0xFF, 0xFF, 0x00, 0x00, 0x00, 0xFA, 0xCF, 0x00, 0xFF, 0xFF,
    ...
    // User-mode RDMSR at 0x8000 (0x0F 0x32).
    mem.write_slice(0x8000, &[0x0F, 0x32]);
    ...
    cpu.write_sreg(sreg::SS, 0x0023, &mem);
    cpu.stack_size_32 = true;
    cpu.write_r32(1, 0x10); // ECX = IA32_TSC — would have succeeded from CPL=0
    cpu.write_r32(r16::SP as u8, 0x2000);
    cpu.ip = 0x8000;
    cpu.flags = 0x0202;

    let mut io = IoBus::new();
    cpu.step(&mut mem, &mut io).expect("step RDMSR");

    // #GP gate dispatched — would have returned TSC value otherwise.
    assert_eq!(cpu.sregs[sreg::CS], 0x0008);
    assert_eq!(cpu.ip, 0x9000);
    // EAX/EDX untouched (CPL=0 path would have written EAX = TSC low,
    // EDX = TSC high; the #GP path doesn't run that write).
    assert_eq!(cpu.read_r32(0), 0);
    assert_eq!(cpu.read_r32(2), 0);
    assert_eq!(mem.read_u32(0x3000 - 20), 0x8000);
}

/// MOV CR3, EAX from CPL=3 raises #GP(0). A userspace process
/// reloading CR3 would relocate the kernel's page directory —
/// classic privilege-escalation primitive. Real silicon traps it;
/// we used to silently succeed.
#[test]
fn mov_cr3_in_ring_3_raises_gp() {
    let mut mem = Memory::new(0x10_0000);
    let gdt: [u8; 48] = [
        0, 0, 0, 0, 0, 0, 0, 0, 0xFF, 0xFF, 0x00, 0x00, 0x00, 0x9A, 0xCF, 0x00, 0xFF, 0xFF, 0x00,
        0x00, 0x00, 0x92, 0xCF, 0x00, 0xFF, 0xFF, 0x00, 0x00, 0x00, 0xFA, 0xCF, 0x00, 0xFF, 0xFF,
    ...
    // User-mode `MOV CR3, EAX` at 0x8000:
    //   0F 22 D8  — modrm = 11 011 000 (reg=3=CR3, rm=0=EAX).
    mem.write_slice(0x8000, &[0x0F, 0x22, 0xD8]);

    let mut cpu = Cpu::new();
    cpu.reset_to_boot();
    cpu.cr0 = 1;
    cpu.cr3 = 0xAAAA_0000; // distinctive original CR3
    cpu.gdtr.base = 0x0500;
    cpu.gdtr.limit = 0x2F;
    cpu.idtr.base = 0x6000;
    cpu.idtr.limit = 0x07FF;
    ...
    cpu.write_sreg(sreg::SS, 0x0023, &mem);
    cpu.stack_size_32 = true;
    cpu.write_r32(0, 0xDEAD_BEEF); // would-be new CR3
    cpu.write_r32(r16::SP as u8, 0x2000);
    cpu.ip = 0x8000;
    cpu.flags = 0x0202;

    let mut io = IoBus::new();
    cpu.step(&mut mem, &mut io).expect("step MOV CR3");

    // #GP gate dispatched.
    assert_eq!(cpu.sregs[sreg::CS], 0x0008);
    assert_eq!(cpu.ip, 0x9000);
    // CR3 untouched — the userspace write didn't land.
    assert_eq!(cpu.cr3, 0xAAAA_0000);
    // Saved IP = start of MOV (fault, not trap).
    assert_eq!(mem.read_u32(0x3000 - 20), 0x8000);
}
