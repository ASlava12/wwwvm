# Milestones & boot/userspace journal

Detailed, dated record of the bring-up — kernel boot, syscall coverage, the
Linux 6.12 i386 boot, and the busybox/ld.so userspace milestones (each with its
regression-test name). Moved out of README.md to keep that navigable; nothing
here is required reading to use the project.

## Userspace syscall milestones (i386 kernel)
