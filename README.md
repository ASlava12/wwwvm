...
**801 тест** зелёный (mem 31 + devices 121 + cpu 428 + vm 145
...
Оставшиеся пункты ниже — это НЕ «дистанция до Alpine», а либо
(a) альтернативный путь загрузки «сырой bzImage сам себя
распаковывает» (мы его сознательно обходим, загружая уже извлечённое
ядро + initramfs), либо (b) second-tier полировка устройств. По
приоритету:
...
| Исполнение 16-битного real-mode setup (`setup.bin`) | очень большой, **опциональный** | Единственный непройденный кусок bzImage-пути: запустить ~16 КиБ 16-битного boot-ASM из `setup.bin` (BIOS-стиль: загрузка в 0x7C00, real→PM-переход силами самого ядра). **Не блокер** — 32-битный boot-протокол выше его обходит (так же делают современные VMM). Нужен только для «грузим как настоящий BIOS-загрузчик»; ценность низкая, т.к. ядро и так грузится. |
...
10-thread параллелизме). Поверх этого **Alpine userspace на
собственном LTS-ядре уже работает** (`linux_userspace_alpine_*`),
включая `apk`, научный Python (numpy/CPython) и GUI/X — см. таблицу
и memory. Текущий цикл закрывает оставшиеся second-tier пункты по
...
Должно вывести 801 пройденный тест на текущий момент. CI
