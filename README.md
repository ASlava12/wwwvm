...
вверх по странице. Heap-extend (брать дополнительные страницы
через `sys_brk(new_addr)` с new_addr > current_brk) можно будет
добавить отдельным milestone'ом в будущем. Ещё ~52 секунды
wall-clock.
...
| `sys_pipe` / `sys_pipe2` возвращают ошибку | средний | Прямой milestone-attempt 29 мая 2026 (`build_initramfs_pipe`, реверт'нут до коммита): /init зовёт `sys_pipe(&fds)` (syscall 42) или `sys_pipe2(&fds, 0)` (331), потом write(fds[1], "PIPE", 4), потом read(fds[0], buf, 4). Симптом — единичный "PIPE" в UART и hang после. Диагноз: sys_pipe вернул -errno (не заполнил fds buffer), fds буфер остался нулевым (исходное состояние), write(fd=0, "PIPE", 4) написал на /dev/console (bidirectional, fd 0 stdin=stdout=/dev/console) → "PIPE" в UART; следующий read(fd=0, …) попал на сломанный stdin-path (см. следующий блокер) и заблокировался до budget exhaustion (≈8.8 минут). Обе версии (sys_pipe 42 и sys_pipe2 331) фейлятся одинаково — значит не calling convention, а сама pipe-инфраструктура ядра не работает в нашей эмуляции. Hypothesis: либо `access_ok`/копи в user buffer ловит -EFAULT по какой-то нестыковке между PHDR p_flags и фактическим PTE write-битом, либо pipe filesystem (anon_inode) не инициализирован для нашего процесса. Каноничный репродьюсер живёт в commit message ниже; рабочий instrumented variant (с пропечатыванием eax sys_pipe'а вместо round-trip) — TODO следующего investigate-тика. Не блокирует core milestone'ы (HELLO, /proc/version, time, getpid, gettimeofday, fork, execve, execve-chain все зелёные), но блокирует shell — busybox требует пайпы. |
...
`linux_userspace_brk_milestone` — см. выше), но до работающего
Alpine userspace ещё дистанция
