...
| Boot stall при /init binary size ∈ {600, 602} | неизвестный | Sparse bad set после 15-проб-bisection'а: 600 и 602 hang, 601 (между ними) и 604+ работают. Root cause ещё не найден. Кернель не доходит до /init exec, застревает в pata_legacy probe. Не задевает user-facing /init (hello=139, proc_version=417, uname-then-hello=444). Минимальный репродьюсер: hello-asm + 461 zero-pad → exactly 600 bytes (`build_initramfs_hello_padded_to_600` в [crates/vm/tests/linux_userspace.rs](crates/vm/tests/linux_userspace.rs)). Никакого uname/marker/buf не нужно. |
