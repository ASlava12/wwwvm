...
| Блокер | Объём | Зачем |
|--------|-------|-------|
| Динамическая линковка (`ld.so`) — busybox segfault'ит при relocation | средний | **29 мая 2026:** статические glibc-бинари работают (`real_static_binary_milestone` — ldconfig грузится, проходит CRT/TLS/malloc, печатает свой вывод, чисто `exit(0)`). Динамические — пока НЕТ. `linux_userspace_dynamic_exec_diag`: hand-assembled /init делает `execve("/bin/busybox", ["busybox","echo","DYNLINK_OK"])` с ld-linux.so.2 + libc/libm/libcrypt в `/lib`. Ядро грузит busybox + интерпретатор, но `ld.so` **null-деreferencе'ит во время relocation**: `busybox[1]: segfault at 0 ip b7f21e9d sp bfc362f8 error 4`, faulting-инструкция `mov eax,[eax]` при eax=0, в цикле обхода списка (`mov eax,[ebp-0x10]; cmp [eax+4],0x7fffffff; pause; mov eax,[eax]` — похоже на обход link_map / relocation-списка, где next-указатель оказался 0). error 4 = user-read not-present. Гипотезы: (a) relocation'ы ld.so применяются неверно → указатель в структуре остаётся 0; (b) auxv/стек, который execve-путь ядра построил для ld.so, частично некорректен (хотя статический CRT читает auxv нормально); (c) что-то в mmap/mprotect сегментов .so. Нужна трассировка ld.so. rotdash (libc-only, `dynamic_binary_diag`) доходит до exit(1) без «error while loading shared libraries» — т.е. libc грузится, но что-то в relocation/инициализации ломается. Инфраструктура готова: `build_cpio_tree`, `build_init_execve_busybox`. Это шлюз к Alpine (всё там динамическое). |
