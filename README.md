...
| Блокер | Объём | Зачем |
|--------|-------|-------|
| ✅ **РЕШЕНО** — Динамическая линковка (`ld.so`): MULTI-LIB glibc работает | средний | **✅ РЕШЕНО (30 мая 2026) — MULTI-LIBRARY ДИНАМИКА РАБОТАЕТ END-TO-END.** `linux_userspace_dynamic_multilib_milestone` (asserting): hand-assembled /init делает `execve("/bin/busybox", ["busybox","echo","DYNLINK_OK"])`; ядро грузит busybox, его `PT_INTERP=/lib/ld-linux.so.2` mmap'ит **три** библиотеки (libc.so.6 + libm.so.6 + libcrypt.so.1), проходит relocation, запускает busybox `echo` → печатает `DYNLINK_OK`, чистый `exitcode=0x00000000` (WIFEXITED 0). Тест ассертит DYNLINK_OK && !segfault && !lib_err && !execve-fail. Дополнительно `busybox_direct_diag` (busybox как /init) реально запускает busybox `init`-applet (UART: respawn'ит getty на /dev/tty2/3/4 → «can't open» т.к. в minimal initramfs только /dev/console — НЕ баг). И `linux_userspace_busybox_sh_milestone`: `busybox sh -c "echo SHELL_OK"` → ШЕЛЛ парсит командную строку, гоняет builtin echo → `SHELL_OK`. То есть РАБОТАЕТ настоящий busybox-shell поверх multi-lib loader'а. А `linux_userspace_busybox_sh_fork_exec_milestone`: `sh -c "/bin/busybox echo FORK_OK"` → шелл делает fork()+execve() ВТОРОЙ копии busybox (свежий ld.so-цикл для child'а) → `FORK_OK`. Т.е. шелл реально ЗАПУСКАЕТ программы (fork+exec динамических бинарей). И `linux_userspace_busybox_pipeline_milestone`: `sh -c "echo PIPE_OK | busybox cat"` → шелл строит pipeline (pipe()+fork()+dup2() обеих стадий) → `PIPE_OK` течёт echo→pipe→cat→консоль. Т.е. РАБОТАЕТ полный shell-I/O-plumbing — суть usable Linux userspace. И `linux_userspace_busybox_awk_milestone`: `busybox awk 'BEGIN{print "AWK_OK_" 6*7}'` → awk-интерпретатор считает `6*7=42` (double-арифметика через x87) и форматирует число→строку → `AWK_OK_42`. Это вскрыло НОВЫЙ баг: awk's number-handling использует **DF memory-формы x87** (`FILD m64`/`FISTP m64` — 64-битный int↔double, плюс `FILD/FIST/FISTP m16`), которых не было в декодере (echo/cat/sh/pipeline их не трогают) → `unimplemented opcode 0xDF at 0073:08062AF3`. Реализованы DF /0 /2 /3 /5 /7 (m16/m64 integer load-store) + 2 teeth-confirmed unit-теста (`fpu_fild_fistp_m64_roundtrip`, `fpu_fild_fist_m16_signed`). После фикса awk печатает `AWK_OK_42` чисто. Урок: реальные workload'ы (awk's интерпретатор) ходят по путям, которых нет у простых applet'ов. И `linux_userspace_busybox_file_io_milestone`: `sh -c 'echo FILE_OK > /fmark; busybox cat /fmark'` → шелл открывает /fmark `O_WRONLY|O_CREAT|O_TRUNC`, dup2'ит на stdout, echo пишет в ФАЙЛ (не в консоль), затем `busybox cat` открывает /fmark `O_RDONLY`, читает назад и печатает `FILE_OK`. Т.е. РАБОТАЕТ open/write/read/close на реальном пути в tmpfs-rootfs — персистентные файлы, не только pipe'ы. И `linux_userspace_busybox_sed_milestone`: `sh -c 'echo foo123 | busybox sed s/[a-z][a-z]*/SED_OK_/'` → sed компилирует BRE `[a-z][a-z]*` (класс символов + квантор `*`), матчит ведущий run строчных букв в «foo123» и заменяет → `SED_OK_123`. Т.е. РАБОТАЕТ движок регулярных выражений (regex compile + substitute) — отдельный тяжёлый code-path от awk-интерпретатора, прошёл чисто (новых CPU-пробелов не вскрыл). И `linux_userspace_busybox_ls_milestone`: `sh -c 'echo x > /LSMARK_FILE; busybox ls -l /'` → ls перечисляет корень (getdents64), делает lstat каждой записи и форматирует mode/nlink/uid/gid/size/mtime в колонки → в листинге появляется свежесозданный `LSMARK_FILE`. Т.е. РАБОТАЕТ VFS-metadata путь (readdir + stat + форматирование времени) — отдельная syscall-поверхность от pipe'ов/regex, прошёл чисто. И `linux_userspace_busybox_shell_arith_milestone`: `sh -c 'n=0; for x in 1 2 3 4; do n=$((n+x)); done; if [ $n -eq 10 ]; then echo SHELL_SUM_10; fi'` → НАСТОЯЩИЙ shell-скриптинг: присваивание переменной, `for`-цикл по списку слов, арифметическое раскрытие `$((n+x))` (накапливает 1+2+3+4=10), `if` с builtin `[`-тестом и целочисленным `-eq`. Маркер печатается ТОЛЬКО если цикл прокрутился 4 раза, арифметика дала ровно 10, и условие истинно → валидирует control-flow И корректность арифметики, а не просто «шелл не упал». Прошёл чисто. И `linux_userspace_busybox_gzip_milestone`: `sh -c 'echo GZIP_RT_OK | busybox gzip -c | busybox gunzip -c'` → данные DEFLATE-сжимаются (LZ77 + Huffman + CRC32 + ISIZE-трейлер), передаются в gunzip, который парсит заголовок, инфлейтит и ВЕРИФИЦИРУЕТ CRC32. Маркер `GZIP_RT_OK` переживает round-trip только если весь bit-manipulation путь (table-driven XOR/shift/rotate в CRC32 + битовый Huffman-кодер) корректен — иначе gunzip сообщил бы CRC error и выкинул данные. Самый тяжёлый CPU-стресс на сегодня, прошёл чисто (новых пробелов не вскрыл). И `linux_userspace_busybox_signal_milestone`: `sh -c "trap 'echo TRAP_OK' TERM; kill -TERM \$\$; echo AFTER"` → валидирует доставку сигналов end-to-end (путь, который построили коммиты sys_rt_sigaction + sys_kill, но ни один тест не гонял целиком). Шелл ставит SIGTERM-хендлер (rt_sigaction), шлёт себе SIGTERM (kill); чтобы маркер напечатался, ядро должно РЕАЛЬНО доставить сигнал в userspace: построить rt_sigframe на user-стеке, перенаправить выполнение в хендлер (sa_restorer-трамплин как адрес возврата), выполнить его, затем обработать rt_sigreturn для восстановления контекста — после чего ash выполняет отложенный trap (`echo TRAP_OK`). Если бы доставка/sigreturn были сломаны, дефолтное действие SIGTERM убило бы шелл (→ init умер → kernel panic, который теперь ловит run_busybox_dynamic). Прошёл чисто → доставка сигналов + sigreturn работают. И `linux_userspace_busybox_md5sum_milestone`: `sh -c 'echo MD5_INPUT | busybox md5sum'` → самая острая bit-level проверка CPU-корректности. В отличие от gzip round-trip (где две ошибки могли бы взаимно сократиться), здесь дайджест запиннен к ТОЧНОМУ значению с хоста (`printf 'MD5_INPUT\n' | md5sum` → `dacfc4c8b9f45aa2d7b1c0f2d9e526b9`). MD5 строится из left-rotation'ов (ROL на разные величины) и 32-битных модульных сложений по 64-элементной таблице синус-констант; любой off-by-one в rotate, неверный carry или shift полностью меняют дайджест (лавинный эффект). Эмулированный busybox воспроизвёл точный 128-битный хеш → rotate/add ALU-путь корректен до бита. И `linux_userspace_busybox_jobcontrol_milestone`: `sh -c 'busybox sleep 0 & busybox sleep 0 & busybox sleep 0 & wait; echo ALL_REAPED'` → конкурентность/job-control: шелл форкает ТРИ фоновых job'а (каждый fork+execve busybox), так что три процесса живут одновременно, затем `wait` блокируется пока ВСЕ не reap'нуты через wait4. Маркер печатается только когда все дети собраны — если бы reaping был сломан (зомби не собран, wait4 не матчит ребёнка, SIGCHLD-учёт неверен), `wait` завис бы навсегда. Прошёл чисто → конкурентный fork/exec + multi-child wait reaping работают. И `linux_userspace_busybox_sort_stress_milestone`: `sh -c 'busybox seq 1 20000 | busybox sort -rn | busybox head -1 | busybox sed s/^/SORTED_MAX_/'` → единственное непротестированное load-измерение (все прочие milestone'ы используют крошечные входы). seq выдаёт 20 000 строк (~120 KB); sort -rn должен забуферить ВСЕ в памяти и сделать полную reverse-numeric сортировку (реальное malloc/brk-давление + ~20k·log20k сравнений, намного больше аллокаций и demand-paging чем однострочные входы); head берёт верх (для корректной reverse-numeric сортировки = максимум 20000); sed добавляет маркер. `SORTED_MAX_20000` доказывает, что sort аллоцировал и обработал весь датасет под нагрузкой И упорядочил корректно. steps=2.4B (vs обычные 2.09B) и 69s wall — подтверждают реальные вычисления. Прошёл чисто. И `linux_userspace_busybox_interactive_milestone`: ИНТЕРАКТИВНЫЙ шелл — команда читается из tty (stdin), а не из `-c`-аргумента. /init execve'ит `busybox sh` БЕЗ `-c`, так что REPL блокируется в read() на fd 0 (консоль); как только /init стартанул, харнесс шлёт `echo INTERACTIVE_OK\n` через UART (`send_input` → 16550 RX → RX IRQ → serial-драйвер ядра → tty line discipline → canonical-mode буфер). read() шелла возвращает строку, ash парсит и выполняет, `INTERACTIVE_OK` появляется на консоли. Это вся цепочка keyboard→shell-ввода, которую обходил каждый предыдущий `sh -c` milestone: UART receive, RX-прерывание, tty input cooking. Прошёл чисто → НАСТОЯЩИЙ интерактивный Linux-шелл работает (новый харнесс `run_busybox_dynamic_stdin` кормит stdin после старта init). **ПРИЧИНА была — SYSEXIT CPL=3 баг** (фикс из 5-го аудита): CPUID рекламирует SEP → glibc идёт через vDSO `sysenter`; SYSEXIT возвращался с RPL=0 → после КАЖДОГО syscall'а userspace бежал на CPL=0 → U/S-бит в #PF был 0 (supervisor) → do_page_fault неверно обрабатывал demand-paging busybox'а (3 либы = много demand-page, в отличие от крошечного rotdash) → corrupt control-flow / saved-EIP (симптом `0x6f622037`/`0x6f622061` — saved EIP становился строкой). Форсинг RPL=3 на SYSEXIT всё починил. (Прежний `rep ret` decode-фикс сам по себе НЕ помог.) Это капстоун всей dynamic-linking ветки. ⬇️ Ниже — исторический трейл расследования (УСТАРЕЛ, оставлен для контекста). **single-lib (rotdash):** `linux_userspace_dynamic_single_lib_milestone` (asserting, не diag) грузит Tinycore `rotdash` (1.7 KB, нужна только libc) как /init: ядро видит `PT_INTERP=/lib/ld-linux.so.2`, грузит интерпретатор, тот mmap'ит `libc.so.6` (единственный `DT_NEEDED`), проходит relocation и прыгает в `_start`→glibc CRT→`main`. Бинарь доходит до НОРМАЛЬНОГО выхода (`exitcode=0x100` → `WIFEXITED`, low-7-bit статуса = 0, не убит сигналом), БЕЗ segfault и БЕЗ «error while loading shared libraries». Тест ассертит `reached_init && !segfault && !lib_err && (code&0x7f)==0`. Доказывает: dynamic loader + relocation + однобиблиотечная загрузка работают (на тех же двух CPU-фиксах, что ниже). Multi-lib (busybox: libc+libm+libcrypt) — отдельный firmware far-call (ниже). **29 мая 2026:** статические glibc-бинари работают (`real_static_binary_milestone` — ldconfig грузится, проходит CRT/TLS/malloc, печатает свой вывод, чисто `exit(0)`). Multi-lib динамические — пока НЕТ. `linux_userspace_dynamic_exec_diag`: hand-assembled /init делает `execve("/bin/busybox", ["busybox","echo","DYNLINK_OK"])` с ld-linux.so.2 + libc/libm/libcrypt в `/lib`. Ядро грузит busybox + интерпретатор, но `ld.so` **null-деreferencе'ит во время relocation**: `busybox[1]: segfault at 0 ip b7f21e9d sp bfc362f8 error 4`, faulting-инструкция `mov eax,[eax]` при eax=0, в цикле обхода списка (`mov eax,[ebp-0x10]; cmp [eax+4],0x7fffffff; pause; mov eax,[eax]` — похоже на обход link_map / relocation-списка, где next-указатель оказался 0). error 4 = user-read not-present. **Изолировано (29 мая):** busybox-direct (kernel-exec'd, БЕЗ execve-стаба, `busybox_direct_diag`) segfault'ит идентично (`ip b7f5ae9d`, тот же `mov eax,[eax]` при eax=0) → execve-стаб НИ ПРИ ЧЁМ. rotdash (libc-only) доходит до clean exit; busybox требует 3 либы (libc+libm+libcrypt) и падает в relocation. Дифференциатор — **несколько библиотек**. **Исключённые гипотезы (decisive experiments):** (1) stale mini-TLB — отключил read/write-TLB кэши целиком, segfault БАЙТ-В-БАЙТ идентичен (тот же ip, тот же step-count) → НЕ TLB; (2) file-backed mmap с НЕнулевым offset'ом — `file_mmap_offset_milestone` (mmap pgoff=1 → читает страницу 1) проходит, возвращает «OFFSETOK» → mmap-offset корректен. **Новый ключ:** ebx (PIC GOT-base) на fault'е = `0x425c` (а не реальный адрес ~`0xb7f5xxxx`): `lea eax,[ebx-0x425c]` дал 0 → deref 0 → CR2=0. И байты faulting-кода (`8d83a4bdffff`, `817804ffffff7f`) НЕ найдены ни в одной .so на диске → исполняется не настоящий text библиотеки (повреждённая code-страница / неверный control-flow), специфично для multi-lib. **РЕШЕНО (часть 1, 30 мая):** instruction-trace (новый `Cpu::enable_pf_trace`) показал: faulting eip исполнялся ДВАЖДЫ подряд — `mov eax,[eax]` (8b 00) при eax=0xb7f56080 (валидный указатель) брал #PF на demand-страницу; наша continue-on-fault модель писала мусор-0 в РЕГИСТР-НАЗНАЧЕНИЯ eax (он же базовый регистр адреса!), потом EIP-rewind retry читал [0] → double-fault at 0. Это ОБЩАЯ форма того же бага, что чинился для REP-строк. **Фикс:** checkpoint GP-регистров + flags в начале инструкции, откат при pending_fault (string/REP сами управляют частичным прогрессом и исключены). Userspace null-deref исчез, ld.so проходит дальше (+100M инструкций). **Остался баг №2:** теперь kernel-oops глубже в линковке — `EIP: 0x00002061` (corrupt kernel IP), CR2=080af857, exitcode=0x09 (SIGKILL), DYNLINK_OK ещё не печатается. **РЕШЕНО (часть 2, 30 мая):** баг №2 найден через wild-jump-трассировку + дизассемблирование vmlinux (распаковал из bzImage): kernel-инструкция `2e ff 1d 00 cf b4 c0` = `lcall *%cs:[0xc0b4cf00]` (FF /3, far indirect call) исполнялась в 32-битном режиме как m16:32, но наш handler читал ТОЛЬКО 16-битный offset → target обрезался до low-16 (0x2061). **Фикс:** FF /3 и FF /5 (far indirect CALL/JMP) теперь учитывают op_size_32 (полный 32-битный offset + selector на +4); unit-тест `pm_far_call_indirect_m16_32_uses_full_offset` (teeth-confirmed). **Осталось (вне scope быстрого фикса):** после обоих фиксов ld.so проходит сильно дальше, но ядро делает far-call в НИЗКУЮ память (~0x9cexx, ниже EBDA) — BIOS-service / real-mode-trampoline путь, которого у firmware-less эмулятора нет (apm=off/pnpbios=off/pnpacpi=off/noapm/idle=halt — НИ ОДИН не убрал far-call в 0x9cexx, так что это не APM/PnP/idle; вероятно real-mode trampoline / reboot-aftermath более раннего user-fault'а в busybox @0x080af857). ЗАКЛЮЧЕНИЕ: dynamic-linking end-to-end упирается в эмуляцию firmware/trampoline — вне scope; ветка закрыта с извлечёнными победами (2 CPU-фикса + boot-stall resolved). Это уже не баг ld.so, а пробел эмуляции firmware/trampoline. Итог investigation: 2 общих CPU-correctness фикса (reg-rollback-on-fault, far-call-width) — ценны сами по себе; полный dynamic-linking end-to-end упирается в firmware-эмуляцию. Инструмент `Cpu::enable_pf_trace` остаётся для будущих расследований. Инфраструктура: `build_cpio_tree`, `build_init_execve_busybox`. Шлюз к Alpine. **ОБНОВЛЕНО (30 мая 2026, после +9 CPU-фиксов из 4 аудитов этой сессии): СТЕНА СДВИНУЛАСЬ — больше НЕ firmware far-call.** Re-run `busybox_direct_diag` с новым trace-триггером на wild instruction-FETCH (`Vm::enable_cpu_pf_trace`): первая аномалия теперь — ядро исполняет длинную последовательность (`0xc0122xxx`→`0xc019dxxx`→`0xc07a84xx`) и **возвращается в user EIP `0xb7eff190` (unmapped, в lib-mmap-регионе)** → fetch #PF (err=0x14: U+I/D), затем каскад до kernel-oops `Code: at 0x6f622037` (ASCII), exitcode=9. Это ret-в-user (iret/sysexit) на несуществующий адрес — НЕ firmware-пробел, а tractable баг paging/relocation/return, специфичный для multi-lib. **Следующие шаги:** (1) увеличить trace-ring чтобы увидеть user-код до входа в ядро (corrupt-EIP vs page-not-mapped); (2) дизассемблировать `0xc07a852b` (vmlinux stripped — нужен raw objdump) чтобы подтвердить инструкцию возврата; (3) проверить mmap-раскладку 3 либ (差 vs single-lib rotdash, который работает). Прежний firmware far-call в `0x9cexx` БОЛЬШЕ НЕ ВОСПРОИЗВОДИТСЯ — его убрал один из 9 фиксов (вероятно far-CALL/RETF-width или reg-rollback). Шлюз к Alpine. |
