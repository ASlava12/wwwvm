...
**Найденный sigreturn bug**: первая версия handler'а вместо
exit'а делала `ret` — должна была pop'нуть kernel-stashed
sa_restorer adress со stack'а, перейти в restorer'е (sys_rt_sigreturn),
ucontext restored, main resume'ит после kill'а. Реальный
результат: handler ran (HANDLER marker), но main NEVER resumed
(DONE marker не появился), и kernel panic'нул с
`exitcode=0x0000000b` (signal 11 = SIGSEGV). То есть после
`ret` где-то segfault. Может быть pretcode на stack'е не
установлен, может sigreturn syscall сам restore'ит bad
context. Workaround: handler exits напрямую, sigreturn не
зовётся вообще. Это пинит **one-way signal delivery** —
sigaction storage + kill queueing + kernel jump to handler —
этого хватает для Ctrl-C патернов где handler просто завершает
программу. Round-trip (sigreturn → main resumes) пока broken
— нужно для shell'ов с reentrant handler'ами. Задокументировано
в blocker table для будущего investigate-тика. Ещё ~53 секунды
wall-clock.
...
| `sys_rt_sigreturn` segfault'ит после handler return | средний | 2026-05-29, в ходе `linux_userspace_signal_milestone`: /init установил SIGUSR1 handler через `sys_rt_sigaction(174)`, послал себе `sys_kill(37)`, handler ran (HANDLER marker появился), потом сделал `ret`. Ожидалось: kernel'ский pretcode push'нул sa_restorer'а на stack, ret pop'ает его, restorer зовёт `sys_rt_sigreturn(173)`, kernel restore'ит ucontext, main resume'ит после kill. Реальный результат: kernel panic с `exitcode=0x0000000b` (signal 11 = SIGSEGV). Где-то в sigreturn pathway случается segfault. Возможные причины: (a) kernel'ский setup_rt_frame не push'ил pretcode на user stack (или push'ил wrong value); (b) sigreturn syscall сам restore'ит bad context (некорректные eip, esp, или EFLAGS); (c) sa_flags = SA_RESTORER не уважается, kernel fallback'ит на vDSO restorer'е которого у нас нет. Repro: `linux_userspace_signal_milestone` с handler делающим `ret` вместо `sys_exit(0)` (см. commit history `ret`-варианта). Workaround в milestone'е: handler exits напрямую, sigreturn не зовётся. Не блокирует production milestones; блокирует reentrant signal handlers (shell job control, glibc signal-safe sleep, любой Ctrl-C handler который должен resume'ить). |
| `sys_pipe2` populate fd-пары layout-зависим (fds не копируются в userspace для некоторых /init layout'ов) | средний | **Точная характеристика (Tinycore 15.x kernel, обе пробы на ОДНОМ ядре, один тик):** Две диагностики, оба /init вызывают `sys_pipe2(&fds, 0)` первым делом и читают fds сразу: (1) `linux_userspace_pipe_diag` (bare, только pipe2 + emit) → `ret=0, fds=[0, 4]` — fds **populated корректно** (read end 0, write end 4); (2) `linux_userspace_pipe_rt_diag` (round-trip, с захватом всех return'ов + hex-dump) → `ret=0, fds=[0, 0]` — **fds[1] НЕ populated** (остался zero-init). Hex-dump подтверждает raw байты: `F0=00000000 F1=00000000`. Тот же kernel, тот же syscall, два разных /init binary'я → разный результат. Значит это **НЕ kernel-version баг** (старый 5.85MB ядро тоже давало [0,4]) и **НЕ простой marker-misread** (round-trip честно прочитал raw [0,0] из stream'а). Это **layout/address-зависимая аномалия**: pipe2 возвращает 0 (success), но его `copy_to_user` fd-пары приземляется на fds_addr у одного /init layout'а и не приземляется у другого. Последствие round-trip'а: fds=[0,0], fd 0 = bidirectional /dev/console, поэтому write(fds[1]=0, "PIPE") → console (UART, ret 4), read(fds[0]=0) → console stdin → **блокируется**. Это объясняет "PIPE в UART + hang" из прошлых попыток. **Нужно CPU-level расследование** copy_to_user-during-syscall (не tweak теста). **Следующий точный эксперимент**: в pipe_rt pre-touch'ить fds-страницу (записать байт в fds_addr ДО pipe2) — если это чинит populate, root cause = "copy_to_user в нетронутую data-страницу во время syscall'а не приземляется". Bounded пробы (~52s pass / ~2min если read блокируется): `linux_userspace_pipe_diag` + `linux_userspace_pipe_rt_diag`. Не блокирует production milestones; блокирует shell pipelines (`cmd1 \| cmd2`). |
...
+ `linux_userspace_chdir_milestone` — см. выше; все production
