...
exec'а — пинит полный wall-clock путь до userspace.
...
**571 тест** зелёные (mem 30 + devices 77 + cpu 320 + vm 127 +
...
| Boot stall при /init binary size ∈ {600, 602} | неизвестный | Sparse bad set после 17-проб-bisection'а (включая 608 как контр-пример mod-8 гипотезе): 600 и 602 hang, 601 (между ними), 604, 605, 608 работают. Root cause ещё не найден. Кернель не доходит до /init exec, застревает в pata_legacy probe. Не задевает user-facing /init (hello=139, proc_version=417). Минимальный репродьюсер: hello-asm + 461 zero-pad → exactly 600 bytes (`build_initramfs_hello_padded_to_600` в [crates/vm/tests/linux_userspace.rs](crates/vm/tests/linux_userspace.rs)). Никакого uname/marker/buf не нужно. |
| `sys_read(stdin)` не доставляет байт после `send_input` | средний | Симметричная половина к рабочему write-пути: `vm.send_input(b"K\n")` после того, как /init блокируется в read(0, buf, 1) — ядро ЭХОИТ K обратно в UART (значит UART rx → kernel ISR → ldisc input работает), но buf остаётся нулевым; /init печатает `\0` вместо `K`. Дамп после первого probe-теста: `00003fe0: ... 4541 443f 5d4b 0d0a` (READ?]K\r\n — это echo), затем `00003ff0: ... 474f 543d 00` (GOT=\0 — это write(1, buf, 1) с пустым buf). Гипотеза: ldisc input буфер vs echo path в этом сценарии расходятся — либо ICANON-режим/dev/console не вкладывает символ в read buffer когда нет ожидающего reader'а в момент IRQ, либо есть race между sys_read и нашей ISR. Воспроизводится через test-локальный builder `build_initramfs_echo` (был добавлен в ходе investigate-tick'а 29 мая 2026, revert'нут до коммита). Существующий `WWWVM_INIT_INPUT=Q` в example linux_boot теоретически делает то же самое; нужно проверить, рабочий ли он на самом деле или там тоже buf остаётся нулевым. |
...
+ `linux_userspace_time_milestone` — см. выше), но до работающего
...
Должно вывести 571 пройденный тест на текущий момент. CI
