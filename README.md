...
| Блокер | Объём | Зачем |
|--------|-------|-------|
| x87 расширения — остались 80-бит m80 (FLD/FSTP extended-precision) + FPU-исключения/precision control | малый | **Трансцендентные РЕАЛИЗОВАНЫ (29 мая 2026):** FSIN, FCOS, FSINCOS, FPTAN, FPATAN, F2XM1, FYL2X, FYL2XP1, FSCALE, FXTRACT, FPREM, FPREM1, FDECSTP, FINCSTP (D9 F0..FF) — считаются в f64, C2 сбрасывается как «reduced». 13 unit-тестов (`fpu_fsin_fcos`, `fpu_fyl2x`, `fpu_fpatan`, …). Осталось: 80-битная extended precision (m80 load/store — мы храним ST как f64, так что long double теряет точность) и FPU-исключения (#MF, маски в CW). glibc math по большей части использует SSE2 на современных таргетах, так что это second-tier. |
...
`linux_userspace_pipeline_milestone` — см. выше; все production
