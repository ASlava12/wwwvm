...
* **devices** — 16550 UART (COM1: 0x3F8), 8259A PIC (master, 0x20/0x21),
  8254 PIT (0x40-0x43) и PS/2 keyboard (0x60/0x64). PIC моделирует
  IMR/IRR/ISR, ICW1/ICW2 (ICW3/ICW4 принимаются и отбрасываются),
  non-specific EOI через OCW2. UART: IER на offset+1, `irq_pending()`
  true при rx + IER bit 0. PIT — канал 0 в режимах 0/2/3,
  control-word на 0x43. Keyboard: очередь scan-кодов, port 0x60 read
  pops byte, port 0x64 status bit 0 = OBF; level-triggered IRQ 1 пока
  очередь не пуста. `IoBus::refresh_irqs` смешивает уровневые
...
* Тестов — **129 зелёных** (mem 4 + devices 20 + cpu 89 + vm 10 + wasm 1
...
| Реальные устройства: slave PIC (IRQ 8..15), RTC (CMOS), IDE/ATA, VGA | средний | Загрузочные тракты любого реального дистрибутива |
