...
| `sys_getcwd` возвращает 0 (не в Linux-контракте) | средний | 29 мая 2026 при попытке `linux_userspace_chdir_milestone` (revert'нут до коммита, оставлен как `linux_userspace_chdir_diag`): /init делает `sys_mkdir("/sub", 0o755)` → `sys_chdir("/sub")` → `sys_getcwd(buf, 32)`. Все три синтаксически корректны. Результат: buf весь нулевой, **eax getcwd'а = 0x00000000** — ни positive length (для "/sub\0" должно быть 5), ни negative -errno. Linux sys_getcwd (fs/d_path.c) ВСЕГДА возвращает либо ≥ 2 (для "/" + NUL) либо negative — 0 не в контракте. Гипотезы: (a) syscall 183 в нашем kernel build'е mapped на что-то другое (не sys_getcwd); (b) syscall запускается но eax mishandled на возврате; (c) build_initramfs_chdir имеет address calculation bug который я не нашёл. Diagnostic test `linux_userspace_chdir_diag` (ignored, ~52s) логирует и cwd_bytes, и getcwd ret для последующих ticks. Не блокирует production milestones; блокирует программы которые ходят по каталогам (cd, bash $PWD, getcwd-based path resolution). |
...
+ `linux_userspace_symlink_milestone` — см. выше; все 32
production milestone'а re-verified зелёными на Tinycore 15.x
kernel'е при 10-thread параллелизме), но до работающего Alpine
userspace ещё дистанция по таблице выше. Текущий цикл закрывает
блокеры по одному с тестом на каждый шаг.
