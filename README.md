...
— см. выше; все production milestone'ы re-verified зелёными на
Tinycore 15.x kernel'е при 10-thread параллелизме), но до
работающего Alpine userspace ещё дистанция по таблице выше.
Текущий цикл закрывает блокеры по одному с тестом на каждый шаг.
